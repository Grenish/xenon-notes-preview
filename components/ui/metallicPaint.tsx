...
export function parseLogoImage(
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = function () {
      ...
      // Fix for SVG handling
      let width = img.naturalWidth || img.width;
      let height = img.naturalHeight || img.height;

      // For SVGs, explicitly set dimensions if naturalWidth/naturalHeight are not reliable
      if (file.type === "image/svg+xml") {
        width = MAX_SIZE;
        height = MAX_SIZE;
      }

      // Ensure width and height are valid finite numbers
      ...
      // Final sanity check to ensure dimensions are valid
      width = Math.round(Math.max(1, Math.min(MAX_SIZE, width)));
      height = Math.round(Math.max(1, Math.min(MAX_SIZE, height)));
      ...
      // Draw the image with the corrected dimensions
      try {
      ...
      // Now the image should be properly drawn at valid dimensions
      try {
